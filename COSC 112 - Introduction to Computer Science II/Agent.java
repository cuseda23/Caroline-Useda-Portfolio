...
public class Agent {
    ...
    public boolean prefers (Agent a, Agent b) {
        ...
        if(prefList.contains(a) && !prefList.contains(b) | b==null){
            return true;
        }
        if(a==null && !prefList.contains(b)){
            return true;
        }
        if(prefList.indexOf(a) < prefList.indexOf(b)) {
            return true;
        }
        return false;
    }
    ...
    public void refusal () {
        curIndex = curIndex + 1;
        if(curMatch==null){ //if they havent proposed
            setCurMatch(prefList.get(0));
            prefList.get(0).proposal(this);
        } else {
            if(curIndex >= prefList.size() ){ //if they dont have any other preferences
                setCurMatch(null);
                return;
            }
            Agent next = prefList.get(curIndex);
            if(next != null) {
                setCurMatch(next);
                next.proposal(this);
            } else {
